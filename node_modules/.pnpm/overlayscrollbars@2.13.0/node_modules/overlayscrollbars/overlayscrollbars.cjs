...
const isDefaultDirectionScrollCoordinates = ({M: t, D: n}) => {
...
const getScrollCoordinatesPercent = ({M: t, D: n}, o) => {
    ...
    return addEventListener(h, "pointerdown", (o => {
      ...
      if (x) {
        ...
        const k = [ addEventListener(p, n, releasePointerCapture), addEventListener(p, "selectstart", (t => preventDefault(t)), {
          I: false
        }), addEventListener(h, n, releasePointerCapture), M && addEventListener(h, "pointermove", (t => C(T + (t[s] - $)))), M && (() => {
...
const createScrollbarsSetup = (t, n, o, s, e, c) => {
  ...
  const {Bt: z, Pt: I, Nt: A, qt: T, jt: M} = m;
  ...
  const D = [ S, h, w, v, () => f(), addEventListener(C, "pointerover", onHostMouseEnter, {
    T: true
  ...
  return [ () => bind(runEachAndClear, push(D, O())), ({zt: t, At: n, Qt: e, Zt: c}) => {
    const {tn: r, nn: d, sn: p, en: v} = c || {};
    const {Ct: g, ft: h} = e || {};
    const {j: w} = o;
    const {k: y, U: S} = getEnvironment();
    const {cn: m, F: O} = s;
    const [C, D] = t("showNativeOverlaidScrollbars");
...
const createOverflowUpdateSegment = (t, s) => {
  ...
  const [L, U] = createCache({
    i: (t, n) => equalXY(t.M, n.M) && equalXY(t.D, n.D),
  ...
  return ({zt: n, Qt: o, un: l, At: a}, {_n: u}) => {
    ...
    assignDeep(s, {
      ...
      F: ut,
      Mt: sanitizeScrollCoordinates(ht, et)
    });
    return {
      sn: St,
      tn: st,
  ...
  if (n && !r) {
    ...
    const S = {
      ...
      state() {
        const {vn: t, gn: n} = b();
        const {j: o} = t;
        const {Vt: s, Rt: e, cn: c, F: i, rn: l, fn: a, Mt: u} = n;
        return assignDeep({}, {
          overflowEdge: s,
          overflowAmount: e,
          overflowStyle: c,
    ...
    push(i, [ y ]);
    addInstance(c, S);
    registerPluginModuleInstances(D, OverlayScrollbars, [ S, u, l ]);
